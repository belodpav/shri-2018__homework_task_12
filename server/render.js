...
function render(req, res, data, context) {
    ...
    const bundleName = isMobile ? 'touch' : 'desktop';
    const pathToBundle = path.resolve(__dirname, '..', bundleName + '.bundles', bundleName);


    let templates = getTemplates(pathToBundle, bundleName);
    ...
    if (isDev) templates = getTemplates(pathToBundle, bundleName);
...
function getTemplates(pathToBundle, bundleName) {
    return {
